...
/// Encode a single database to RDB bytes for a given DB index.
fn encode_database(db_idx: usize) -> std::io::Result<Vec<u8>> {
    let mut buf = Vec::new();
    {
        ...
        for (key, value) in guard.iter().filter(|(_, v)| !v.is_expired()) {
            ...
            if let Some(expire_at) = value.expire_at {
                buf.write_all(&[0xFC])?;
                buf.write_all(&expire_at.to_be_bytes())?;
            }
            // Write key
            encode_string(&mut buf, key)?;
            // Write value
            encode_redis_data(&mut buf, &value.data)?;
        }
    }
    Ok(buf)
}

fn encode_string(buf: &mut Vec<u8>, s: &str) -> std::io::Result<()> {
    encode_length(buf, s.len() as u64)?;
    buf.extend_from_slice(s.as_bytes());
    Ok(())
}

fn encode_length(buf: &mut Vec<u8>, len: u64) -> std::io::Result<()> {
    if len < 64 {
        buf.push(len as u8);
    } else if len < 16384 {
        let bits = (len << 2) | 0b01;
        buf.push(0xC0 | ((bits >> 8) as u8));
        buf.push(bits as u8);
    } else {
        let bits = (len << 2) | 0b10;
        buf.push(0xC0 | ((bits >> 24) as u8));
...
fn encode_redis_data(buf: &mut Vec<u8>, data: &crate::storage::data::RedisData) -> std::io::Result<()> {
    match data {
        ...
        crate::storage::data::RedisData::List(items) => {
            buf.push(1); // type: list
            encode_length(buf, items.len() as u64)?;
            for item in items {
                encode_string(buf, item)?;
            }
        }
        crate::storage::data::RedisData::Hash(map) => {
            buf.push(2); // type: hash
            encode_length(buf, map.len() as u64)?;
            for (k, v) in map.iter() {
                encode_string(buf, k)?;
                encode_string(buf, v)?;
            }
        ...
        crate::storage::data::RedisData::SortedSet(zmap) => {
            buf.push(4); // type: sorted set
            encode_length(buf, zmap.len() as u64)?;
            for (member, score) in zmap.iter() {
                encode_string(buf, member)?;
                let mut ryu_buf = ryu::Buffer::new();
                let score_str = ryu_buf.format(*score);
                encode_string(buf, score_str)?;
            }
        }
        crate::storage::data::RedisData::Stream(stream) => {
            buf.push(5); // type: stream
            encode_length(buf, stream.len() as u64)?;
            for (id, entry) in stream.iter() {
                encode_string(buf, id)?;
                encode_length(buf, entry.fields.len() as u64)?;
